var TableEditable15 = function () {

    var handleTable15 = function () {
        ...
        function saveRow(oTable, nRow) {
            ...
            oTable.fnUpdate(jqInputs[0].value+"&nbsp;"+tick_image, nRow, 0, false);
        ...
        var tableWrapper = $("#sample_editable_15_wrapper");
        ...
        $('#sample_editable_15_new').click(function (e) {
            e.preventDefault();
...
//                var errors = check_validation_errors(jqInputs)
                var errors = false;
                console.log(jqInputs.first());
                if(jqInputs.first().val().trim()==='')
                    {
                        errors = true;
                        jqInputs.first().tooltip({title: "Blank Field.", placement: "bottom"}).tooltip("show");
                    }
                if (!errors){
                    ...
                    var aData = oTable.fnGetData(nRow);
                    var terminal_id = aData[4]
                    if (!terminal_id) {
                        saveRowInDB(jqInputs[0].value, jqInputs[1].value);
                    } 
                    else {
                        updateRowInDB(jqInputs[0].value, jqInputs[1].value, terminal_id);
                    }
                    // Magic ends
                }
